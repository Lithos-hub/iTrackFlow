...
const getScaleNotes = ({ rootNote, scaleType }: { rootNote: Note; scaleType: ScaleType }) => {
	switch (scaleType) {
		...
		case 'augmented':
			...
			break;
		case 'chromatic':
			if (rootNote === 'C') return C_CHROMATIC_POSITIONS;
			if (rootNote === 'D') return D_CHROMATIC_POSITIONS;
			if (rootNote === 'E') return E_CHROMATIC_POSITIONS;
			if (rootNote === 'F') return F_CHROMATIC_POSITIONS;
			if (rootNote === 'G') return G_CHROMATIC_POSITIONS;
			if (rootNote === 'A') return A_CHROMATIC_POSITIONS;
			if (rootNote === 'B') return B_CHROMATIC_POSITIONS;
			if (rootNote === 'C#' || rootNote === 'Db') return C_SHARP_CHROMATIC_POSITIONS;
			if (rootNote === 'D#' || rootNote === 'Eb') return D_SHARP_CHROMATIC_POSITIONS;
			if (rootNote === 'F#' || rootNote === 'Gb') return F_SHARP_CHROMATIC_POSITIONS;
			if (rootNote === 'G#' || rootNote === 'Ab') return G_SHARP_CHROMATIC_POSITIONS;
			if (rootNote === 'A#' || rootNote === 'Bb') return A_SHARP_CHROMATIC_POSITIONS;
			break;
		case 'ionian':
...
const getScaleSemitones = (scaleType: string) => {
	switch (scaleType) {
		case 'major':
